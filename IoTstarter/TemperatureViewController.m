...
- (void)displayTemperature:(NSData *)dataBytes {
    ...
    double ambientTempC = ((double)rawAmbientTemp);
    double ambientTempF = [self fahrenheitFromCelsius:ambientTempC];
    NSLog(@"*** Light level: %hu", rawAmbientTemp);
    ...
    self.temperatureLabel.text = [NSString stringWithFormat:@"%hu", rawAmbientTemp];
}
...
- (void)displayHumidity:(NSData *)dataBytes {
    ...
    [dataBytes getBytes:&rawHumidity length:dataLength * sizeof(uint16_t)];
    //double calculatedHumidity = calculateRelativeHumidity(rawHumidity);
    NSLog(@"*** Activity Level: %hu%%", rawHumidity);
    self.humidityLabel.text = [NSString stringWithFormat:@"Activity Level: %hu%%", rawHumidity];
